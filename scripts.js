        ...
        function applyTemplate() {
            ...
            const bh = selectedStyle === 'ranktag' ? 16 : 14, iw = selectedStyle === 'ranktag' ? 15 : 14, tw = textWidth, gap = 1;
            const sx = Math.floor((64-(iw+gap+tw))/2), sy = selectedStyle === 'ranktag' ? 24 : Math.floor((64-bh)/2);
            
            const drawBox = (x, y, bw) => {
                if(style === 'ranktag') {
                    const outline = shadeColor(baseColor, -85);
                    const edgeDark = shadeColor(baseColor, -55);
                    const edgeMid = shadeColor(baseColor, -20);
                    const mid = baseColor;
                    const midLight = shadeColor(baseColor, 32);
                    const hi = shadeColor(baseColor, 70);
                    const low = shadeColor(baseColor, -38);
                    baseCtx.fillStyle = outline; baseCtx.fillRect(x, y, bw, bh);
                    baseCtx.fillStyle = '#FFFFFF'; baseCtx.fillRect(x+1, y+1, bw-2, bh-2);
                    baseCtx.fillStyle = hi; baseCtx.fillRect(x+2, y+2, bw-4, bh-4);
                    baseCtx.fillStyle = midLight; baseCtx.fillRect(x+3, y+3, bw-6, bh-6);
                    baseCtx.fillStyle = hi; baseCtx.fillRect(x+3, y+3, bw-6, 2);
                    baseCtx.fillStyle = shadeColor(baseColor, 46); baseCtx.fillRect(x+3, y+5, bw-6, 2);
                    baseCtx.fillStyle = mid; baseCtx.fillRect(x+3, y+7, bw-6, 2);
                    baseCtx.fillStyle = edgeMid; baseCtx.fillRect(x+3, y+9, bw-6, 2);
                    baseCtx.fillStyle = low; baseCtx.fillRect(x+3, y+11, bw-6, 2);
                    baseCtx.fillStyle = edgeDark; baseCtx.fillRect(x+3, y+13, bw-6, 1);
                    baseCtx.fillStyle = 'rgba(255,255,255,0.55)'; baseCtx.fillRect(x+4, y+4, Math.max(6, Math.floor((bw-8) * 0.55)), 1);
                    baseCtx.fillStyle = shadeColor(baseColor, 62); baseCtx.fillRect(x+4, y+6, Math.max(4, Math.floor((bw-8) * 0.35)), 1);
                    return;
                }
                if(style === 'hollow') {
            ...
            if(icon) {
                ...
                drawMatrixIcon(baseCtx, icon, sx + 1, sy + 1, iw - 2, bh - 2, iCol, useIShadow, isx, isy, isCol, document.getElementById('iconGradient').checked);
            }
            previewEditor();
        }
